defmodule Trackguests3Web.VisitorLive.CheckIn do
  ...
  def handle_event("search_rooms", %{"person" => %{"room_search" => search_term}}, socket) do
    ...
    {:noreply,
     ...
     |> maybe_clear_selected_room(search_term)}
  end
  ...
  def handle_event("select_room", %{"room-id" => room_id, "room-title" => room_title, "room-floor" => room_floor}, socket) do
    ...
    # Update the formdefmodule Trackguests3Web.VisitorLive.CheckIn do
  use Trackguests3Web, :live_view
              ...
              <div class="relative">
                ...
                <div class="relative">
                  <.input
                    field={@form[:room_search]}
                    type="text"
                    placeholder="Start typing room number or name..."
                    phx-keyup="search_rooms"
                    autocomplete="off"
                    class="input-luxury w-full text-gray-900 placeholder:text-gray-500 bg-gray-50 border-2 border-gray-300 focus:border-gray-500 focus:ring-4 focus:ring-gray-200 rounded-xl px-6 py-4 font-medium transition-all duration-300"
                    required
                ...
                <%= if @show_room_suggestions and length(@filtered_rooms) > 0 do %>
                  <div class="absolute z-50 w-full mt-2 bg-white border-2 border-gray-200 rounded-xl shadow-2xl max-h-64 overflow-y-auto">
                    <%= for room <- @filtered_rooms do %>
                      <button
                        type="button"
                        phx-click="select_room"
                        ...
                        phx-value-room-title={room.title}
                        phx-value-room-floor={room.floor}
                        class="w-full text-left px-6 py-4 hover:bg-gray-50 border-b border-gray-100 last:border-b-0 transition-colors duration-200 group"
                      >
                ...
                <%= if @show_room_suggestions and length(@filtered_rooms) == 0 and @room_search_query != "" do %>
                  <div class="absolute z-50 w-full mt-2 bg-white border-2 border-gray-200 rounded-xl shadow-2xl p-6 text-center">
                    <div class="text-gray-500">
                      <svg class="w-8 h-8 mx-auto mb-2 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                      </svg>
                      <p class="text-sm">No rooms found matching "{@room_search_query}"</p>
                    </div>
                  </div>
                <% end %>
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> assign(:rooms, rooms)
     |> assign(:filtered_rooms, [])
     |> assign(:show_room_suggestions, false)
     |> assign(:room_search_query, "")
     |> assign(:selected_room, nil)
     |> assign(:form, to_form(changeset))
     |> assign(:form_valid, false)}
  end
  ...
  @impl true
  def handle_event("search_rooms", %{"value" => query}, socket) do
    ...
    filtered_rooms = 
      if query == "" do
        ...
        socket.assigns.rooms
        |> Enum.filter(fn room ->
          # Search by room title or floor number
          String.contains?(String.downcase(room.title), String.downcase(query)) or
          String.contains?(to_string(room.floor), query)
        end)
        |> Enum.take(8)  # Limit to 8 results for better UX
      end

    {:noreply,
     socket
     |> assign(:filtered_rooms, filtered_rooms)
     |> assign(:show_room_suggestions, query != "")
     |> assign(:room_search_query, query)}
  end

  @impl true
  def handle_event("select_room", %{"room-id" => room_id, "room-title" => room_title, "room-floor" => room_floor}, socket) do
    # Update the form with selected room
    changeset = 
      %Person{}
      |> Persons.change_person(%{
        "room_search" => "#{room_title} - Floor #{room_floor}",
        "room_id" => room_id
      })

    # Re-validate the form with the new room selection
    ...
    {:noreply,
     ...
     |> assign(:show_room_suggestions, false)
     |> assign(:filtered_rooms, [])
     |> assign(:selected_room, %{id: room_id, title: room_title, floor: room_floor})
