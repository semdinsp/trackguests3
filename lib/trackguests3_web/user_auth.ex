defmodule Trackguests3Web.UserAuth do
  ...
  @remember_me_cookie "_trackguests3_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      Trackguests3Web.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      Trackguests3Web.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule Trackguests3Web.PageLive do
        use Trackguests3Web, :live_view

        on_mount {Trackguests3Web.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{Trackguests3Web.UserAuth, :require_authenticated}] do
  ...
  def require_admin_user(conn, _opts) do
    if conn.assigns.current_scope && conn.assigns.current_scope.user &&
         Accounts.admin?(conn.assigns.current_scope.user) do
      conn
    else
      conn
